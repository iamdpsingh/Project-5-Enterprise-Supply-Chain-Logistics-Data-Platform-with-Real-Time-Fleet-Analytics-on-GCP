...
def execute_sql_file(client: bigquery.Client, filepath: str):
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"Executing: {filename} ({len(statements)} statements)")
    logger.info(f"{'='*60}")
    ...
    for i, stmt in enumerate(statements, 1):
        ...
        logger.info(f"  [{i}/{len(statements)}] Running...")
        query_job = client.query(stmt)
        query_job.result()
        bytes_processed = query_job.total_bytes_processed or 0
        logger.info(f"  [{i}/{len(statements)}] Done ({bytes_processed} bytes processed)")
